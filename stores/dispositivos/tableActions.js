...
export const fetchTableData = async () => {
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[table-action] Error al obtener los datos:', response);
      return;
    }
    if (!success) {
    ...
    console.error('[table-action] Error al obtener los datos:', error.message);
    console.error(error);
...
export const handleEdit = async id => {
  try {
    ...
    if (!res.ok) {
      notify('Error al intentar obtener los datos', 'error');
      console.error('[table-action] Error al obtener los datos:', res);
      return;
    }
    const { success, response } = await res.json();
    if (!success) {
...
export const handleDelete = async id => {
  try {
    const disp = $tableData.get().find(d => d.id === id);
    if (!disp) {
      console.error('[table-action] Error al eliminar los datos:', res);
      return;
    }
    const conf = confirm(`¿Está seguro de eliminar el dispositivo ${disp.rio} (${disp.ubicacion})?`);
    if (!conf) {
    ...
    notify(`Se eliminó el dispositivo ${disp.rio} (${disp.ubicacion})`, 'success');
  } catch (error) {
    console.error('[table-action] Error al eliminar los datos:', error.message);
    console.error(error);
  ...
  try {
    timer = setTimeout(async () => {
      ...
      if (!res.ok) {
        ...
        console.error('[table-action] Error al buscar:', res);
        return;
      }
      const { success, response } = await res.json();
      if (!success) {
    ...
    console.error('[tableAction] Error al buscar:', error.message);
    console.error(error);
    notify('Error al intentar buscar los datos', 'error');
  }
};
